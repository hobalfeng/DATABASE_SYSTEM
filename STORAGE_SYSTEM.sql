...
--
-- 替換檢視表以便查看 `view_inventory_status`
-- (請參考以下實際畫面)
--
CREATE TABLE `view_inventory_status` (
...
--
-- 替換檢視表以便查看 `view_product_flow`
-- (請參考以下實際畫面)
--
CREATE TABLE `view_product_flow` (
`product_name` varchar(200)
,`start_area` varchar(50)
,`end_area` varchar(50)
,`move_qty` int(11)
,`move_time` datetime
...
--
-- 替換檢視表以便查看 `view_purchase_performance`
-- (請參考以下實際畫面)
--
CREATE TABLE `view_purchase_performance` (
`進貨時間` datetime
,`供應商` varchar(100)
,`經辦人員` varchar(100)
,`產品名稱` varchar(200)
...
--
-- 替換檢視表以便查看 `view_shipment_details`
-- (請參考以下實際畫面)
--
CREATE TABLE `view_shipment_details` (
`shipment_id` varchar(50)
,`shipment_time` datetime
,`customer_name` varchar(100)
,`product_name` varchar(200)
,`shipped_quantity` int(11)
,`user_id` varchar(50)
,`staff_name` varchar(100)
);
...
--
-- 檢視表結構 `view_inventory_status`
--
DROP TABLE IF EXISTS `view_inventory_status`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_inventory_status`  AS SELECT `p`.`name` AS `product_name`, `s`.`stock_quantity` AS `quantity`, `l`.`area` AS `area`, `l`.`location_type` AS `location_type` FROM ((`product` `p` join `stock` `s` on(`p`.`product_id` = `s`.`product_id`)) join `location` `l` on(`s`.`location_id` = `l`.`location_id`)) ;

-- --------------------------------------------------------

--
-- 檢視表結構 `view_product_flow`
--
DROP TABLE IF EXISTS `view_product_flow`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_product_flow`  AS SELECT `p`.`name` AS `product_name`, `l1`.`location_type` AS `start_area`, `l2`.`location_type` AS `end_area`, `pm`.`quantity` AS `move_qty`, `pm`.`move_time` AS `move_time`, `u`.`username` AS `staff_name` FROM ((((`product_movement` `pm` join `product` `p` on(`pm`.`product_id` = `p`.`product_id`)) join `location` `l1` on(`pm`.`start_location` = `l1`.`location_id`)) join `location` `l2` on(`pm`.`end_location` = `l2`.`location_id`)) join `user` `u` on(`pm`.`user_id` = `u`.`user_id`)) ;

-- --------------------------------------------------------

--
-- 檢視表結構 `view_purchase_performance`
--
DROP TABLE IF EXISTS `view_purchase_performance`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_purchase_performance`  AS SELECT `p`.`purchase_time` AS `進貨時間`, `p`.`supplier` AS `供應商`, `u`.`username` AS `經辦人員`, `pr`.`name` AS `產品名稱`, `pd`.`quantity` AS `進貨數量` FROM (((`purchase` `p` join `purchase_detail` `pd` on(`p`.`purchase_id` = `pd`.`purchase_id`)) join `product` `pr` on(`pd`.`product_id` = `pr`.`product_id`)) join `user` `u` on(`p`.`user_id` = `u`.`user_id`)) ;

-- --------------------------------------------------------

--
-- 檢視表結構 `view_shipment_details`
--
DROP TABLE IF EXISTS `view_shipment_details`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_shipment_details`  AS SELECT `s`.`shipment_id` AS `shipment_id`, `s`.`shipment_time` AS `shipment_time`, `s`.`customer_name` AS `customer_name`, `p`.`name` AS `product_name`, `sd`.`quantity` AS `shipped_quantity`, `u`.`user_id` AS `user_id`, `u`.`username` AS `staff_name` FROM (((`shipment` `s` join `shipment_detail` `sd` on(`s`.`shipment_id` = `sd`.`shipment_id`)) join `product` `p` on(`sd`.`product_id` = `p`.`product_id`)) join `user` `u` on(`s`.`user_id` = `u`.`user_id`)) ;
